...
def safe_user_label(user: Any) -> str:
    name = obj_get(user, "full_name") or obj_get(user, "first_name") or obj_get(user, "username") or "—"
    return html.escape(safe_text(name))


def build_ad_preview_caption(ad_data: dict[str, Any], user: Any) -> str:
    user_id = obj_get(user, "id", 0)
    username = obj_get(user, "username")
    try:
        vip_flag = is_vip(int(user_id))
    except Exception:
        vip_flag = False
    return (
        "📄 <b>Նախադիտում</b>\n\n"
        f"👤 Ուղարկող՝ <b>{safe_user_label(user)}</b>\n"
        f"🆔 ID՝ <code>{safe_text(user_id)}</code>\n"
        f"Username՝ @{html.escape(safe_text(username)) if username else '—'}\n\n"
        f"{build_ad_text(ad_data, vip_badge=vip_flag)}\n\n"
...
def build_payment_admin_caption(payment_id: int, user: Any, package: str, amount: int, ad_id: Optional[int]) -> str:
    ...
    return (
        ...
        f"🆔 ID՝ <code>{safe_text(user_id)}</code>\n"
        f"Username՝ @{html.escape(safe_text(username)) if username else '—'}\n"
        f"📦 Տեսակ՝ <b>{html.escape(safe_text(package))}</b>\n"
...
async def post_ad_to_channel(ad_id: int, ad_data: dict[str, Any], user: Any) -> list[int]:
    try:
        user_id = int(obj_get(user, "id", 0))
    except Exception:
        user_id = 0
    text = build_ad_text(ad_data, vip_badge=is_vip(user_id))
    photos = list(ad_data.get("photos", []))
    if not photos:
