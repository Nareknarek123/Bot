...
def build_vip_text(user_id: int) -> str:
    remaining = 30 - (get_ref_count(user_id) % 30) if get_ref_count(user_id) % 30 != 0 else 30
    return (
        "💎 <b>VIP բաժին</b>\n\n"
        "💰 Գին — <b>30000 դրամ</b>\n"
        "Ստանում եք՝\n"
        ...
        f"Քո referral-ը — <b>{get_ref_count(user_id)}</b>\n"
        f"Մնացել է մինչև հաջորդ credit — <b>{remaining}</b>\n"
        f"Քո credit-ը — <b>{get_ref_credits(user_id)}</b>\n"
    )
...
def build_ad_preview_caption(ad_data: dict[str, Any], user: Any) -> str:
    return (
        "📄 <b>Նախադիտում</b>\n\n"
        f"👤 Ուղարկող՝ <b>{html.escape(user.full_name)}</b>\n"
...
async def refresh_vip_ads_loop() -> None:
    while True:
        try:
            rows = db("SELECT * FROM ads WHERE status='approved' AND vip_active=1", fetchall=True) or []
            for ad in rows:
...
async def ad_approve(callback: CallbackQuery) -> None:
    ...
    try:
        ...
        if is_vip(ad["user_id"]):
            try:
                await bot.pin_chat_message(CHANNEL_ID, msg_ids[0], disable_notification=True)
            except Exception:
                pass
            mark_ad_vip(ad_id, True)
...
async def vip_use_ref(callback: CallbackQuery) -> None:
    ...
    await callback.message.answer(
        "🎁 Ձեր 1 referral credit-ը ակտիվ է։\nԿարող եք օգտագործել այն որպես՝\n📌 1 զակրեպ կամ 🔄 1 թարմացում\n\nՄանրամասների համար կապվեք ադմինին։",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="💬 Կապ ադմինին", url=ADMIN_URL)]]),
    )
    await callback.answer("Credit օգտագործվեց", show_alert=False)
...
async def pay_approve(callback: CallbackQuery) -> None:
    ...
    if package == "vip":
        ...
        if ad_id:
            db("UPDATE ads SET vip_active=1 WHERE id=?", (ad_id,))
    elif package == "pin":
        if ad_id:
